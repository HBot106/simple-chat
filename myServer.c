...
void waitForConnection(int serverSocket)
{
	...
	//at init, tail = head
	struct socketDescriptor_t* tailSocket = HEAD_SOCKET;
	...
	while (1)
	{
		...
		checkForActiveSocket(HEAD_SOCKET, tailSocket, &selectionSet);
	}
}
...
void checkForActiveSocket(struct socketDescriptor_t* currentSocket, struct socketDescriptor_t* tailSocket, fd_set* selectionSet)
{
	if (FD_ISSET(currentSocket->fileDescriptor, selectionSet))
	{
		...
		//the server socket is active; accept new connection
		{
			...
			newClientSocket->previous_socketDescriptor = currentSocket;
			tailSocket->next_socketDescriptor = newClientSocket;
			tailSocket = newClientSocket;
	...
	{
		checkForActiveSocket(currentSocket->next_socketDescriptor, tailSocket, selectionSet);
	}
}
...
void flag05_IN(struct commonChatHeader_t* header, struct socketDescriptor_t* socket)
{
	fprintf(stdout, "%s: %s\n", "[DEBUG]", "flag 5 in");
	...
	// "header->additionalFields" points to the byte after the flag of the common header
	// in this case, that is the length of the handle
	uint8_t sourceHandleLength = (uint8_t)*header->additionalFields;
	
	//extract the sourceHandle
	memcpy(sourceHandle, &header->additionalFields[1], sourceHandleLength);
	//null terminate the handle for easier printing
	sourceHandle[sourceHandleLength] = '\0';

	//extract the numberOfDestinationHandles
	memcpy(&numberOfDestinationHandles, &header->additionalFields[(1 + sourceHandleLength)], 1);

	//validated destination to send to
	struct socketDescriptor_t* validDestination;

	//maintains position in the header
	char* locationInHeader = &header->additionalFields[(2 + sourceHandleLength)];
	...
	for (i = 0; i < numberOfDestinationHandles; i++)
	{
		...
		if (validDestination)
		{
			flag05_OUT(header, validDestination);
		}
		//otherwise return an invalid handle packet to the source
		else
		{
			flag07_OUT(header, socket, currentHandle, currentHandleLength);
		}
...
//send message packet
void flag05_OUT(struct commonChatHeader_t* header, struct socketDescriptor_t* validDestination)
{
	fprintf(stdout, "%s: %s\n", "[DEBUG]", "flag 5 out");
	// no need to rebuild packet
	if (send(validDestination->fileDescriptor, header, ntohs(header->length), 0) < 0)
	{
		perror("send call");
		exit(-1);
	}
...
void flag07_OUT(struct commonChatHeader_t* header, struct socketDescriptor_t* socket, char* invalidHandle, uint8_t invalidHandleLength)
{	
	fprintf(stdout, "%s: %s\n", "[DEBUG]", "flag 7 out");
	//build the packet
...
void flag08_IN(struct socketDescriptor_t* socket, fd_set* selectionSet)
{
	fprintf(stdout, "%s: %s\n", "[DEBUG]", "flag 8 in");
	...
	FD_CLR(socket->fileDescriptor, selectionSet);
