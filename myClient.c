...
void flag07_IN(uint8_t* receiveBuffer)
{
	...
	fprintf(stderr, "Client with handle %s does not exist", handle);
	exit(-1);
}
...
void flag04_SENDER(char* inputBuffer, int bufferPosition)
{
	...
	while (messageLength > MAX_MESSAGE_LENGTH)
	{
		...
		memcpy(&sendBuf[4 + handleLength], &inputBuffer[bufferPosition], MAX_MESSAGE_LENGTH - 1);
		memcpy(&sendBuf[4 + handleLength + (MAX_MESSAGE_LENGTH - 1)], &myNull, 1);
	...
	if (messageLength <= MAX_MESSAGE_LENGTH)
	{
		...
		memcpy(&sendBuf[4 + handleLength + (messageLength - 1)], &myNull, 1);
